...
def generate_inference_json_finer_resolution(wav_filename, output_dir, segment_duration=2.0, skip=0.1, sample_rate=16000):
    """
...
def save_to_json(output_dict, output_json):
    """
    Save json files to a output json file name.
    Args:
        output_dict (dict): Output content to json files.
...
def process_wav_folder(folder_path, output_dir):
    ...
    inference_2s_total={}
    inference_2s_finer_total={}

    for wav_file in wav_files:
        wav_path = os.path.join(folder_path, wav_file)

        print(f"Processing: {wav_path}")
        inference_2s=generate_inference_json_2s(wav_path, output_dir)
        inference_2s_finer=generate_inference_json_finer_resolution(wav_path, output_dir)
        
        inference_2s_total.update(inference_2s)
        inference_2s_finer_total.update(inference_2s_finer)

    save_to_json(inference_2s_total, os.path.join(output_dir, "inference_data.json"))
    save_to_json(inference_2s_finer_total, os.path.join(output_dir, "inference_data_finer_resolution.json"))
