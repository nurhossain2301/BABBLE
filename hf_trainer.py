...
import sys
sys.path.append('./')
from raven.model import *
from raven.constants import NUM_FRAMES, IGNORE_INDEX, MODAL_INDEX_MAP
from raven.mm_utils import tokenizer_multimodal_token, process_video, process_image, process_audio_file
from raven.raven_trainer import (RAVENTrainer,
...
class ModelArguments:
    # LLM Arguments
    model_type: Optional[str] = field(default="raven", metadata={"help": "Model type selected in the list: " + ", ".join(VLLMs.keys())})
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    mm_alingment_module_lr: Optional[float] = None
...
class LBAudioDatasetMiddleFrame:
    ...
    def __getitem__(self, idx):
        ...
        message=self.tokenizer.apply_chat_template(message, tokenize=False, add_generation_prompt=False)
        input_ids = self.llm_tokenizer(message, add_special_tokens=False, padding="longest", truncation=True, return_tensors='pt').input_ids
        # print(len(input_ids[0]), len(input_ids[1]))
        # input_ids = torch.tensor(input_ids, dtype=torch.long)
        target_ids = copy.deepcopy(input_ids[-1])

        ##changed by nur

        instruction = self.llm_tokenizer.apply_chat_template(message[:1], tokenize=False, add_generation_prompt=True)
        conversation = self.llm_tokenizer.apply_chat_template(message, tokenize=False, add_generation_prompt=False)
        instruction_len = len(self.llm_tokenizer(instruction, add_special_tokens=False, padding="longest", truncation=True, return_tensors='pt').input_ids)
        conversation_len = len(self.llm_tokenizer(conversation, add_special_tokens=False, padding="longest", truncation=True, return_tensors='pt').input_ids)
        return waveform, message, self.data_keys[idx]
...
def train(attn_implementation=None):
    ...
    # for p in model.get_model().mm_allingment_module.parameters():
    #     p.requires_grad = True
